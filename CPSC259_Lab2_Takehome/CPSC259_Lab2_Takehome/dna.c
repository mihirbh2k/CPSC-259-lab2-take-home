...
void analyze_segments(char* sample_segment, char** candidate_segments, int number_of_candidates, char* output_string)
{
    ...
    for (i = 0; i < number_of_candidates; i++) {
        if (strstr(candidate_segments[i], sample_segment) && strlen(sample_segment) == strlen(candidate_segments[i])) {
...
int calculate_score(char* sample_segment, char* candidate_segment)
{
  ...
  //we made this one below
  int ignore = sample_length % 3;

  char* candidatecopy = (char*)malloc(candidate_length * sizeof(candidate_segment));
  if (candidatecopy != NULL) {
      strcpy(candidatecopy, candidate_segment);
  } //just did that if statement to avoid a warning

  char* samplecopy = (char*)malloc(sample_length * sizeof(candidate_segment));
  if (samplecopy != NULL) {
      strcpy(samplecopy, sample_segment);
  } //same as above

  for (int i = 0; i <= candidate_length - sample_length + ignore; i+=3) {  
      for (int j = 0; j < sample_length - ignore; j += 3) {
          if (candidatecopy != NULL && samplecopy != NULL) { //done to avoid warnings, same as before
              char codon1[4];
              strncpy(codon1, (candidatecopy+j+i), 3);
              char codon2[4];
              strncpy(codon2, (samplecopy+j), 3);
              codon1[3] = '\0';
              codon2[3] = '\0';
              if (strcmp(codon1, codon2) == 0) {
                  temp_score += 10;
              }
              else if (strcmp(codon_names[get_codon_index(codon1)], codon_names[get_codon_index(codon2)])==0) { 
                  temp_score += 5;
              }
              else {
                  ...
                  else if (is_base_pair(codon1[0], codon2[0])) {
                      temp_score += 1;
                  }
                  ...
                  else if (is_base_pair(codon1[1], codon2[1])) {
                      temp_score += 1;
                  }
                  ...
                  else if (is_base_pair(codon1[2], codon2[2])) {
                      temp_score += 1;
                  }
              }
          }
